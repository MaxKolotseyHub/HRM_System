...
namespace HRM_System_Bll.Services
{
    public class VacationService : IVacationService
    {
        ...
        public int GetAvailableDays(int id)
        {
            ...
            var daysPerYear = 30;
            var thisYear = vacations.Where(x => x.EndDate.HasValue && x.EndDate.Value.Year == DateTime.Now.Year);
            var usedDays = thisYear.Sum(x => ((DateTime)x.EndDate - (DateTime)x.StartDate).Days);
